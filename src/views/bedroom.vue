...
<script>
    ...
    export default {
		...
		methods:{
			getBedroomList(flag){
                axios.get("/bedrooms").then((result)=>{
                    var res = result.data;
                    this.bedroomList = res.result.list;
            ...
            sortPrice(flag){
                ...
                axios.get("/bedrooms/price",{
					...
					if (res.status == '0') {
                        this.sortType = true;
                        if (flag) {
                            this.bedroomList = this.bedroomList.concat(res.result.list);
