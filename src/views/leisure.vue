...
<script>
    ...
    export default {
		...
		methods:{
			getLeisureList(){
				axios.get("/leisures").then((result)=>{
                    var res = result.data;
                    this.leisureList = res.result.list;
            ...
            sortPrice(flag){
                ...
                axios.get("/leisures/price",{
					...
					if (res.status == '0') {
                        this.sortType = true;
                        if (flag) {
                            this.leisureList = this.leisureList.concat(res.result.list);
