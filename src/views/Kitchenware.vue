...
<div class="shop_list">
    ...
    <div class="list_item">
    <ul>								
        <li v-for="item in KitchenwareList">
...
<script>
    ...
    export default {
		...
		methods:{
			getKitchenwareList(flag){
                axios.get("/kitchenwares").then((result)=>{
                    var res = result.data;
                    this.KitchenwareList = res.result.list;
            ...
            sortPrice(flag){
                ...
                axios.get("/kitchenwares/price",{
					...
					if (res.status == '0') {
                        this.sortType = true;
                        if (flag) {
                            this.KitchenwareList = this.KitchenwareList.concat(res.result.list);
            ...
            addCart(proID){
                ...
                axios.post("/kitchenwares/addCart",{
                    proID:proID,
                    userId:userId
                }).then((res)=>{
