...
<script>
    ...
    export default {
		...
		methods:{
			getWineList(){
				axios.get("/wines").then((result)=>{
                    var res = result.data;
                    this.wineList = res.result.list;
            ...
            sortPrice(flag){
                ...
                axios.get("/wines/price",{
					...
					if (res.status == '0') {
                        this.sortType = true;
                        if (flag) {
                            this.wineList = this.wineList.concat(res.result.list);
